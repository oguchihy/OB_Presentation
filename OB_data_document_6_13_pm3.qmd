...
# List of variables to be used for stratification
strat_vars <- c("zip_93905", "zip_93906", "zip_93907", "zip_93908", "zip_93925",
                "membrane_rupture_None", "intrapartal_events_Precip_Labor_lt3_hrs",
                "intrapartal_events_Prolonged_Labor_gt20_hrs", "uds_age_25_44",
                "conditions_cnsldt_Placenta_Previa", "conditions_cnsldt_Preeclampsia",
                "conditions_cnsldt_Preeclampsia_Infection", "conditions_cnsldt_Prolonged_ROM",
                "lbr_type_cnsldt_Induced", "lbr_type_cnsldt_Not_Applicable",
                "lbr_type_cnsldt_Spontaneous", "hghRsk_yes", "cluster_1")

# Function to fit Cox model and extract p-value for stratification variable
extract_pval_from_survdiff <- function(strat_var) {
  # Add the stratification variable to the data frame
  ob_data_encoded$strat_var <- factor(ifelse(ob_data_encoded[[strat_var]] == 1, "Present", "Absent"))
  
  # Perform the survival analysis
  surv_fit <- survfit(Surv(time, event) ~ strat_var, data = ob_data_encoded)
  
  # Perform the log-rank test
  surv_diff <- survdiff(Surv(time, event) ~ strat_var, data = ob_data_encoded)
...
# Fit models and compile p-values
p_values <- sapply(strat_vars, extract_pval_from_survdiff)

# Compile results into a data frame
results_df <- data.frame(
  strat_var = strat_vars,
  p_value = p_values
)
...
significant_results <- rownames_to_column(significant_results)[-1]
colnames(significant_results) <- c("stratified_variable", "p_value")
