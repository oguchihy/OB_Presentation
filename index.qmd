...
Average birth weight was **```r ave_birth_wt```**, with minimum **```r min_birth_wt```** and maximum of **```r max_birth_wt```**.
...
Let's now shift our attention to hospital events related to OB care. First, we will extend the previous analyses to hospital events. Then, we will use a different method of analysis known as Survival Analysis (SA) or by Time-to-Event (TTE) Analysis to probe our data.
...
Delivery Type: 30% induced; 14% spontaneous -- greater association with labor induction 
Intrapartal conditions: None 16.6% versus 100% Preclampsia, 18.1% infection, and 17.6 % Prolonged Rupture of Membrane; this means that the presence of these conditions (100- 16.6 % for None), Preeclampsia, Infection and PROM are more associated with High RIsk OB
Intrapartal Events: None 21.9%, Prolonged Labor > 20 hrs 15 27.8%	-- High association with intrapartal events (100-21.9%) and prolonged labor
...
(@) Hazard: **"Risk of Event (Delivery) at any point in time** at any given Time
...
```
Hazard Ratio calculations show that among conditions or variables with a statistically significant ration, the self-referential relative risk or likelihood of Delivery at any point in time is higher with the following conditions:
Increasing gestational age (not by much), higher APGAR 1 score, maternal age group 25-44 years, absence of membrane rupture, spontaneous labor, and precipitous labor less than 3 hours; 
while the relative likelihood or risk of delivery was lower in: Placenta Previa, Prolonged Rupture of Membrane, Preeclampsia with or without infection, Prolonged labor greater than 20 hours, labor induction, AGPAR 5, increasing maternal age (not by much), zip codes 95076 and 93927.
...
Summary of Significant Odds Ratios from Logistic Regression Models
Significant Variables and Their Interpretation:
...
Summary of Multinomial Logistic Regression Results
The multinomial logistic regression model was used to analyze the relationship between various predictors and the occurrence of either Intrapartal Events or Intrapartal conditions. The outcome variable was re-coded to model the presence of intrapartal events as "Yes" versus "None."
...
Significance:
Predictors with significant p-values (e.g., p < 0.05) are considered to have a statistically significant association with the outcome.
...
Feature Selection: Variable importance can be used for feature selection. Less important variables (those with low MeanDecreaseGini values) can be removed from the model to simplify it without significantly impacting its performance.
