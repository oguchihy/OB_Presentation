...
Primary Cesarean section rate was **```r prim_c_section_prcnt```%**, constituting **`r prim_c_section_prcnt_all.cs`%** of all c-sections.  Repeat C-Section rate was **```r repeat_cs_prcnt```%**. Total C-section rate was **```r c_section_prcnt```%**. There was **1 Fetal Demise** during the period.
...
para_zip_plt.hb <- as.data.frame(print(strata_tbl_para_zip, cramVars = "para_fct")) %>%
  slice(-1:-2) %>%
  select(-p, -test)
...
**Of note: there is no significant variation by zip code.**
...
For only the first ever delivery alone, here is the outcome:
...
Regarding pregnancies without delivery disparity __at the time of the first delivery__, the data may show a pattern (See plot above) of more at younger ages for 93905; more for later age groups (29 and over) for 93930; more at both the youngest and oldest age groups for 93927; while 93906 has least at both the youngest and oldest age groups, the middle age groups having the most.
...
There are many other ways to analyze the data with this method. For example, the question was posed: _does labor induction lead to more C-sections?_
...
- What is the comparative likelihood of delivery having occurred given a certain condition at a certain time (Hazard Ratio, (HR))?
