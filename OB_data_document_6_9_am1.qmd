...
forecast_historical <- forecast %>% filter(ds <= max(prophet_data$ds))
forecast_future <- forecast %>% filter(ds > max(prophet_data$ds))

# Plot the original data and the forecast
ggplot() +
  geom_line(data = prophet_data, aes(x = ds, y = y, color = "Historical Data"), size = 1) +
  geom_line(data = forecast_historical, aes(x = ds, y = yhat, color = "Fitted Data"), size = 1) +
  geom_line(data = forecast_future, aes(x = ds, y = yhat, color = "Forecasted Data"), linetype = "dashed", size = 1) +
  geom_ribbon(data = forecast_future, aes(x = ds, ymin = yhat_lower, ymax = yhat_upper, fill = "Forecast Confidence Interval"), alpha = 0.5) +
  labs(title = "Weekly Deliveries Forecast",
       x = "Week",
       y = "Number of Deliveries",
       color = "Legend",
       fill = "Legend") +
  scale_color_manual(values = c("Historical Data" = "black", "Fitted Data" = "blue", "Forecasted Data" = "blue")) +
  scale_fill_manual(values = c("Forecast Confidence Interval" = "lightblue")) +
  theme_minimal() +
  theme(
