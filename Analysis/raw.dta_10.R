...
model <- lm(delivery_birthweight_num2 ~ age + gestational_days + city + zip + initial_trimester + uds_age, data = mdl.ds.lm, na.action = na.exclude)
summary(model)
...
glm_model <- glm(delivery_birthweight_num2 ~ age + gestational_days + city + zip + initial_trimester 
                 + uds_age, data = mdl.ds.glm, family = gaussian())
...
rf_model <- randomForest(delivery_birthweight_num2 ~ age + gestational_days + city + zip + initial_trimester
                         + uds_age, data = mdl.ds.rf, ntree = 500, importance = TRUE)
...
gbm_model <- gbm(delivery_birthweight_num2 ~ age + gestational_days + city + zip + initial_trimester + uds_age,
                 data = mdl.ds.gbm, 
...
formula <- delivery_birthweight_num2 ~ age + gestational_days + city + zip + initial_trimester + uds_age
...
# 
# library(fastshap)
# X_prepared <- ob_data_filtered[, c("age", "gestational_days", "city", "zip", "initial_trimester", "uds_age")]
# 
...
# X_prepared$initial_trimester <- as.factor(X_prepared$initial_trimester)
# X_prepared$uds_age <- as.factor(X_prepared$uds_age)
# 
# # Compute SHAP values
...
#by City
ob_del_trnd %>%
    ggplot(aes(x = episode_indicator_date_dt, y = delivery_birthweight_num2)) +
