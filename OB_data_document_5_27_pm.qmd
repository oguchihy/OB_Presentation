...
Let's now shift our attention to hospital events related to OB care.
...
cox_model <- coxph(Surv(time, event) ~ zip + del_method_cnsldt + hghRsk + conditions_cnsldt + intrapartal_events + age + grav + para, data = ob_data_srvl)
#summary(cox_model)
...
# Fit Cox Proportional Hazards Model with Left-Censoring
cox_model_left_censor <- coxph(Surv(time, event) ~ age + strata(zip) + del_method_cnsldt + hghRsk + conditions_cnsldt + intrapartal_events, data = filtered_data)
