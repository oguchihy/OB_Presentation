...
strata_tbl_events_clstr <- CreateTableOne(data = ob_data_tbl1, strata = "intrapartal_events", vars = "cluster")
...
# cox_model <- coxph(Surv(time, event) ~ zip, data = ob_data_srvl)
# #summary(cox_model)
# 
...
# library(survminer)
# 
# # Ensure zip is a factor
# ob_data_srvl$zip <- as.factor(ob_data_srvl$zip)
# 
# # Identify and correct invalid rows
# invalid_rows <- ob_data_srvl$time <= 0
# ob_data_srvl <- ob_data_srvl[!invalid_rows, ]
# 
# # Fit the Cox model (using zip as a covariate)
# fit <- coxph(Surv(time, event) ~ zip + hghRsk, data = ob_data_srvl)
# 
# # Plot adjusted survival curves using ggsurvplot_adjusted_curves
# ggsurvplot_adjusted_curves(fit, data = ob_data_srvl, variable = "zip")
# 
# 
# 
# ###############
...
# Fit Cox Proportional Hazards Model
cox_model_hr <- coxph(Surv(time, event) ~ uds_age + cluster + del_method_cnsldt + hghRsk + conditions_cnsldt + intrapartal_events + lbr_type_cnsldt, data = filtered_data)
#summary(cox_model_hr)
#myRFunctions::ggforest3(cox_model_hr)
# ggforest(cox_model_hr)
ggadjustedcurves(cox_model_hr)
# Extract hazard ratios and confidence intervals
hr_table <- summary(cox_model_hr)$coefficients
...
ggforest3 <- function(cox_model_hr, data, main = "Hazard Ratios", fontsize = 0.7) {
  library(myRFunctions)
  
  # Extract coefficients and their confidence intervals
  coef_df <- summary(cox_model_hr)$coefficients
...
hr_plot <- ggforest3(cox_model_hr, filtered_data)
print(hr_plot)
...
# Fit Cox Proportional Hazards Model
cox_model <- coxph(Surv(time, event) ~ zip + del_method_cnsldt + hghRsk + conditions_cnsldt + intrapartal_events, data = filtered_data)
#summary(cox_model)
...
suppressWarnings({
model <- glm(binary_outcome ~ zip_category + age + grav + para + gender + uds_age + hghRsk + del_method_cnsldt, 
...
# Fit the logistic regression model (assuming model is already fitted as in your context)
suppressWarnings({
model_hgh <- glm(binary_outcome_hgh ~ zip_category + age + grav + para + gender + uds_age + del_method_cnsldt + cluster, 
            data = ob_data_fctr, family = binomial())
})
#summary(model_hgh)
