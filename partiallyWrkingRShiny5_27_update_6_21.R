...
server <- function(input, output, session) {
  ...
  output$survPlot <- renderPlot({
    ...
    if (input$strataVar != "None" && !is.null(ob_data_reg[[input$strataVar]])) {
      strata_levels <- levels(factor(ob_data_reg[[input$strataVar]]))
  ...
  output$hrPlot <- renderPlot({
    fit <- fitModel()
    ggforest(fit, data = ob_data_reg, fontsize = 1.2) + 
