...
#STL decompositon superimposition
...
One important goal of data analysis is to make future forecasts. Based on the weekly delivery numbers for 117 weeks, we can predict the following quarter's performance:
...
# mae_value <- mae(prophet_data$y, fitted_values)
# mse_value <- mse(prophet_data$y, fitted_values)
...
# Convert historical data to tsibble for decomposition
historical_ts <- ob_data_ts %>%
  as_tsibble(index = week_start)

# Decompose the historical time series
decomposition <- historical_ts %>%
  model(STL(deliveries ~ trend(window = 13) + season(window = "periodic"))) %>%
  components()

# Plot 1: Historical data with trend and seasonality
p1 <- ggplot() +
  geom_line(data = historical_ts, aes(x = week_start, y = deliveries, color = "Historical Data"), size = 1) +
  geom_line(data = decomposition, aes(x = week_start, y = trend, color = "Trend"), size = 1) +
  geom_line(data = decomposition, aes(x = week_start, y = season_adjust, color = "Seasonality"), size = 1) +
  labs(title = "Historical Data with Trend and Seasonality",
       x = "Week",
       y = "Number of Deliveries",
       color = "Legend") +
  scale_color_manual(values = c("Historical Data" = "black", "Trend" = "blue", "Seasonality" = "red")) +
  theme_minimal() +
  theme(
...
# Convert combined data to tsibble for forecast decomposition
combined_data_tsibble <- combined_data %>%
  as_tsibble(index = ds)

# Decompose the combined time series
combined_decomposition <- combined_data_tsibble %>%
  model(STL(y ~ trend(window = 13) + season(window = "periodic"))) %>%
  components()

# Add 'type' information back to the decomposed data
combined_decomposition <- combined_decomposition %>%
  mutate(type = combined_data$type)

# Plot 2: Forecast with trend and seasonality
p2 <- ggplot() +
  geom_line(data = combined_data_tsibble, aes(x = ds, y = y, color = type), size = 1) +
  geom_line(data = combined_decomposition %>% filter(type == "Historical"), aes(x = ds, y = trend), color = "blue", size = 1) +
  geom_line(data = combined_decomposition %>% filter(type == "Historical"), aes(x = ds, y = season_adjust), color = "red", size = 1) +
  geom_line(data = combined_decomposition %>% filter(type == "Forecast"), aes(x = ds, y = trend, color = "Forecast Trend"), linetype = "dashed", size = 1) +
  geom_line(data = combined_decomposition %>% filter(type == "Forecast"), aes(x = ds, y = season_adjust, color = "Forecast Seasonality"), linetype = "dashed", size = 1) +
  labs(title = "Forecast with Trend and Seasonality",
       x = "Week",
       y = "Number of Deliveries",
       color = "Legend") +
  scale_color_manual(values = c("Historical" = "black", "Forecast" = "orange", "Forecast Trend" = "blue", "Forecast Seasonality" = "red")) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    axis.title = element_text(size = 12),
...
# Delivery number Predictions using Time Series
...
# Sample dataset creation for demonstration (remove if ob_data_chr is already loaded)
# ob_data_chr <- data.frame(
#   delivery_date = seq(as.Date("2020-01-01"), as.Date("2023-01-01"), by = "day"),
#   event = sample(0:1, size = 1096, replace = TRUE)
# )

# Summarize deliveries by week
ob_data_ts_1 <- ob_data_chr %>% 
  dplyr::mutate(delivery_date = as.Date(delivery_date),
         week_start = floor_date(delivery_date, "week")) %>%
  group_by(week_start) %>% 
  summarise(deliveries = sum(event, na.rm = TRUE)) %>%
  ungroup()

# Prettify time series plot without interactivity
p1 <- ob_data_ts_1 %>% 
  ggplot(aes(x = week_start, y = deliveries)) +
  geom_line(color = "blue") +
  geom_point(color = "red") +
  labs(title = "Weekly Deliveries",
       x = "Week",
       y = "Number of Deliveries") +
  theme_minimal() +
  theme(
...
ob_data_tsibble_1 <- ob_data_ts_1 %>%
  as_tsibble(index = week_start)

# Decompose the time series
decomposition_1 <- ob_data_tsibble_1 %>%
  model(STL(deliveries ~ trend(window = 13) + season(window = "periodic"))) %>%
  components()

# Plot trend and seasonality lines along with the original data
p2 <- ggplot() +
  geom_line(data = ob_data_tsibble_1, aes(x = week_start, y = deliveries, color = "Original Data"), size = 1) +
  geom_line(data = decomposition_1, aes(x = week_start, y = trend, color = "Trend"), size = 1) +
  geom_line(data = decomposition_1, aes(x = week_start, y = season_adjust, color = "Seasonality"), size = 1) +
  labs(title = "Trend and Seasonality in Weekly Deliveries",
       x = "Week",
       y = "Value",
       color = "Component") +
  scale_color_manual(values = c("Original Data" = "black", "Trend" = "blue", "Seasonality" = "red")) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
    axis.title = element_text(size = 12),
...
adf_test <- ur.df(ob_data_ts_1$deliveries, type = "trend", selectlags = "AIC")
#summary(adf_test)

# Perform Ljung-Box test on the residuals of the STL decomposition
stl_decomposition_1 <- stl(ts(ob_data_ts_1$deliveries, frequency = 52), s.window = "periodic")
residuals <- stl_decomposition_1$time.series[, "remainder"]

ljung_box_test <- Box.test(residuals, type = "Ljung-Box")
#print(ljung_box_test)

# Fit an ARIMA model
model_fit <- ob_data_tsibble_1 %>%
  model(ARIMA(deliveries))

# Generate forecast for the next quarter (13 weeks)
forecast_horizon <- 13
forecasts_1 <- model_fit %>%
  forecast(h = forecast_horizon)

# Plot the forecasts
forecast_plot_1 <- forecasts_1 %>%
  autoplot(ob_data_tsibble_1, level = NULL) +
  labs(title = "Weekly Deliveries Forecast",
       x = "Week",
       y = "Number of Deliveries",
       color = "Legend") +
  theme_minimal() +
...
# Extract fitted values and calculate accuracy metrics
fitted_values <- model_fit %>%
  augment() %>%
  as_tibble()

#mae <- mae(fitted_values$deliveries, fitted_values$.fitted)
#mse <- mse(fitted_values$deliveries, fitted_values$.fitted)
