...
saveRDS(ob_data_compact, "ob_data_compact.RDS")
...
# HighRisk OB ----
ob_data_hghRsk <- ob_data_compact %>%
    mutate(
        hghRsk = case_when(
...
saveRDS(ob_data_hghRsk, "High Risk OB.RDS")
table(ob_data_hghRsk$hghRsk)
sum(is.na(ob_data_hghRsk))

##Cox_hghRsk----
ob_data_hghRsk$hghRsk <- as.factor(ob_data_hghRsk$hghRsk)


# Fit Cox proportional hazards model
cox_hghRsk_model <- coxph(Surv(adm_to_del_tm, event) ~ hghRsk + weight + gest_age_days, data = ob_data_hghRsk)

# Summary of the Cox model
summary(cox_hghRsk_model)
...
# Fit the Cox model
fit_hghRsk_cox <- coxph(Surv(adm_to_del_tm, event) ~ hghRsk + weight + gest_age_days + strata(hghRsk), data = ob_data_hghRsk)

# Generate adjusted survival curves using the fitted model
# Explicitly pass the data frame used in the model fitting
ggadjustedcurves(fit = fit_hghRsk_cox, data = ob_data_hghRsk)
#ggadjustedcurves(fit = fit_cluster_cox, variable = "cluster_cox", data = ob_data_compact, fun = "cumhaz")

#cumhaz; cum-event
surv_fit <- survfit(Surv(adm_to_del_tm, event) ~ hghRsk + strata(hghRsk), data = ob_data_hghRsk)
# Plotting the Survival Probability
plot(surv_fit, fun = "event", col = 1:3, main = "Survival Probability",
     xlab = "Time", ylab = "Delivery-as-Event Probability", mark.time = TRUE)
legend("bottomright", legend = levels(ob_data_hghRsk$hghRsk), col = 1:3, lty = 1)

# Plotting the Cumulative Hazard
plot(surv_fit, fun = "cumhaz", col = 1:3, main = "Cumulative Hazard",
     xlab = "Time", ylab = "Cumulative Hazard of Delivery_as_Event", mark.time = TRUE)
legend("topleft", legend = levels(ob_data_hghRsk$hghRsk), col = 1:3, lty = 1)


# Compute Kaplan-Meier survival estimate
surv_fit_hghRsk <- survfit(cox_hghRsk_model)
ggsurvplot(surv_fit_hghRsk, data = ob_data_hghRsk, conf.int = FALSE, risk.table = TRUE)

surv_fit_hghRsk <- survfit(cox_hghRsk_model)
ggsurvplot(surv_fit_hghRsk, data = ob_data_hghRsk, conf.int = FALSE, risk.table = TRUE, fun = "event")

ggforest3(cox_hghRsk_model, data = ob_data_hghRsk)

## Fit full Cox High Risk----
##the Cox proportional hazards model with adjusted reference level
cox_hghRsk_full_model <- coxph(Surv(adm_to_del_tm, event) ~  hghRsk +presentation_cnsldt + lbr_type_cnsldt +diff_grav_para + conditions_cnsldt,
                   data = ob_data_hghRsk)

surv_fit_hghRsk_full <- survfit(Surv(adm_to_del_tm, event) ~ strata(hghRsk), data = ob_data_hghRsk)
ggsurvplot(surv_fit_hghRsk_full, data = ob_data_hghRsk, conf.int = FALSE, risk.table = TRUE,surv.median.line = "hv" )

ggsurvplot(surv_fit_hghRsk_full, data = ob_data_hghRsk, conf.int = FALSE, risk.table = TRUE, 
           surv.median.line = "hv", fun = "event")

ggforest3(cox_hghRsk_full_model, data = ob_data_hghRsk)
...
# Fit the multinomial logistic regression model
multinom_model_hghRsk <- multinom(intrapartal_events ~ hghRsk, data = ob_data_hghRsk)

# Summary of the model
summary(multinom_model_hghRsk)

# Coefficients of the model
coef(summary(multinom_model_hghRsk))
...
# Obtain a tidy dataframe of the model's coefficients
tidy_model_hghRsk <- tidy(multinom_model_hghRsk)

library(ggplot2)

# Coefficient plot with the corrected faceting variable
ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = p.value < 0.05)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    facet_wrap(~ y.level, scales = "free") +
    labs(title = "Coefficient plot of Multinomial Logistic Regression",
         x = "Predictors",
...
##2. Intrapartal events only---- 
library(nnet)

library(dplyr)

ob_data_hghRsk_mdl <- ob_data_compact %>%
    mutate(
        hghRsk = case_when(
            age > 35 ~ "yes",  # Age greater than 35
            str_detect(intrapartal_conditions, "Preeclampsia") ~ "yes",  # Presence of Preeclampsia
    ...
    #sum(is.na(ob_data_hghRsk))
    mutate(
        hghRsk = as.numeric(factor(hghRsk, levels = c("no", "yes"))) - 1,  # Convert 'no' -> 0, 'yes' -> 1
        intrapartal_events = factor(intrapartal_events)  # Convert to factor if not already
    )

# Confirm the changes
str(ob_data_hghRsk_mdl$hghRsk)
str(ob_data_hghRsk_mdl$intrapartal_events)
library(dplyr)

library(nnet)

# Fit a multinomial logistic regression model
model_hghRsk <- multinom(intrapartal_events ~ hghRsk, data = ob_data_hghRsk_mdl)

# Summary of the model to see effects
summary(model_hghRsk)
...
predicted_probs <- predict(model_hghRsk, type = "probs")

# Bind the probabilities back to the original data
ob_data_hghRsk_mdl <- bind_cols(ob_data_hghRsk_mdl, as.data.frame(predicted_probs))

# Add 'hghRsk' back for plotting (ensure it is a factor for clear plotting)
ob_data_hghRsk_mdl$hghRsk <- factor(ob_data_hghRsk_mdl$hghRsk, levels = c(0, 1), labels = c("No", "Yes"))

library(tidyr)

# Melt the data for ggplot
plot_data <- ob_data_hghRsk_mdl %>%
    pivot_longer(cols = starts_with("None"):last_col(), names_to = "intrapartal_event", values_to = "probability")

# Since the first column will be 'None' due to alphabetical order, adjust as necessary


ggplot(plot_data, aes(x = hghRsk, y = probability, fill = intrapartal_event)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    labs(x = "High Risk Status", y = "Predicted Probability", title = "Predicted Probabilities of Intrapartal Events") +
    theme_minimal() +
...
# Fit the multinomial logistic regression model
multinom_model_hghRsk <- multinom(intrapartal_events ~ hghRsk, data = ob_data_hghRsk_mdl)

library(broom)

# Tidy the model's coefficients
tidy_model_hghRsk <- tidy(multinom_model_hghRsk)

library(ggplot2)

# Plotting the coefficients
ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = y.level)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    coord_flip() +  # This flips the axes for easier reading of the term labels
    labs(x = "Predictor", y = "Coefficient Estimate", title = "Coefficients from Multinomial Logistic Regression") +
    theme_minimal() +
    scale_fill_brewer(palette = "Set3")  # Adds color for each level of the response
...
library(dplyr)
library(tidyr)
library(broom)  # Ensure broom is loaded for the tidy function

# Get model summary for standard errors
model_summary_hghRsk <- summary(multinom_model_hghRsk)

# Calculate z-values (coefficient divided by standard error)
z_values <- coef(model_summary_hghRsk) / model_summary_hghRsk$standard.errors

# Calculate p-values
p_values <- 2 * (1 - pnorm(abs(z_values)))

# Convert p-values matrix to data frame
p_values_df <- as.data.frame(p_values, check.names = FALSE)

# Assign row and column names from the original matrix
rownames(p_values_df) <- rownames(p_values)
names(p_values_df) <- colnames(p_values)

# Add y.level to p_values_df
p_values_df$y.level <- rownames(p_values_df)

# Convert to long format
p_values_tidy <- pivot_longer(p_values_df, cols = c("(Intercept)", "hghRskYes"), names_to = "term", values_to = "p.value")

# Tidy the model's coefficients
tidy_model_hghRsk <- tidy(multinom_model_hghRsk)

# Merge p-values into the tidy model data
tidy_model_hghRsk <- tidy_model_hghRsk %>%
    left_join(p_values_tidy, by = c("term", "y.level"))

# Optionally, clean up the data frame by selecting and renaming columns
# tidy_model_hghRsk <- tidy_model_hghRsk %>%
    #select(-"p.value.x") %>%
    #rename(p.value = "p.value.y")

# View the first few rows of the merged data frame
#head(tidy_model_hghRsk)


library(ggplot2)

# Plotting the coefficients with p-values
ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = y.level)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    coord_flip() +  # This flips the axes for easier reading of the term labels
    labs(x = "Predictor", y = "Coefficient Estimate", title = "Coefficients and P-values from Multinomial Logistic Regression") +
    theme_minimal() +
    scale_fill_brewer(palette = "Set3") +  # Adds color for each level of the response
    geom_text(aes(label = sprintf("p = %.3f", p.value.y)), position = position_dodge(width = 0.9), hjust = -0.1, size = 3.5)
...
ob_data_logit_hghRsk <- ob_data_hghRsk %>%
    mutate(
        hghRsk = factor(ifelse(hghRsk == "no", "No", "Yes"), levels = c("No", "Yes")),  # Convert hghRsk to factor with levels "No", "Yes"
        intrapartal_events = factor(ifelse(intrapartal_events == "None", "No", "Yes"), levels = c("No", "Yes"))  # Convert intrapartal_events to factor with levels "No", "Yes"
    )

# Confirm the changes
str(ob_data_logit_hghRsk$hghRsk)
str(ob_data_logit_hghRsk$intrapartal_events)

# Fit logistic regression model
logit_model_hghRsk <- glm(intrapartal_events ~ hghRsk, family = binomial, data = ob_data_logit_hghRsk)

# Summary of the model
summary(logit_model_hghRsk)

# Optionally, tidy the model summary for clean output using broom
tidy_logit_hghRsk <- tidy(logit_model_hghRsk)
print(tidy_logit_hghRsk)

# Plotting the estimated coefficients (log odds) and their confidence intervals
ggplot(tidy_logit_hghRsk, aes(x = term, y = estimate)) +
    geom_point() +  # Plot the point estimates of the coefficients
    geom_errorbar(aes(ymin = estimate - 1.96 * std.error, ymax = estimate + 1.96 * std.error), width = 0.2) +  # Error bars for 95% CI
    coord_flip() +  # Flip coordinates for easier reading
    labs(x = "Predictors", y = "Log Odds", title = "Log Odds and 95% Confidence Intervals") +
    theme_minimal() +  # Minimalist theme
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Line at zero for reference
    annotate("text", x = seq_along(tidy_logit_hghRsk$term), y = tidy_logit_hghRsk$estimate, 
             label = paste("p-value =", round(tidy_logit_hghRsk$p.value, 4)), hjust = 0, vjust = -0.5, size = 3)  # Add p-value annotations
...
#                        labels = scales::trans_format("log10", scales::math_format(10^.x))) +
#     annotate("text", x = seq_along(tidy_logit_hghRsk$term), y = tidy_logit_hghRsk$estimate, 
#              label = paste("p-value =", round(tidy_logit_hghRsk$p.value, 4)), hjust = 0, vjust = -0.5, size = 3)
# 
# library(ggplot2)
# 
# # Plotting the coefficients with improved p-value annotations and legend placement
# plot <- ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = y.level)) +
#     geom_bar(stat = "identity", position = position_dodge()) +
#     coord_flip() +  # This flips the axes for easier reading of the term labels
#     labs(x = "Predictor", y = "Coefficient Estimate", title = "Coefficients and P-values from Multinomial Logistic Regression") +
#     theme_minimal() +
#     scale_fill_brewer(palette = "Set3") +  # Adds color for each level of the response
#     geom_text(aes(label = sprintf("p = %.3f", p.value)), position = position_dodge(width = 0.9), hjust = -0.5, vjust = 0.5, size = 3.5, check_overlap = TRUE) +
#     theme(legend.position = "top")  # Moves the legend to the top to avoid blocking p-values
# 
# # Print the plot
# print(plot)
# 
...
# library(ggplot2)
# 
# # Plotting the coefficients with improved p-value annotations, faceting by 'y.level'
# plot <- ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = y.level)) +
#     geom_bar(stat = "identity", position = position_dodge()) +
#     coord_flip() +  # This flips the axes for easier reading of the term labels
#     labs(x = "Predictor", y = "Coefficient Estimate", title = "Coefficients and P-values from Multinomial Logistic Regression") +
...
#     geom_text(aes(label = sprintf("p = %.3f", p.value)), position = position_dodge(width = 0.9), hjust = -0.5, vjust = 0.5, size = 3.5, check_overlap = TRUE) +
#     theme(legend.position = "top") +  # Moves the legend to the top to avoid blocking p-values
#     facet_wrap(~ y.level)  # Facet by categories of 'intrapartal_events'
# 
# # Print the plot
# print(plot)
...
##ranges ----
dte_range_NMC_data <- range(ob_data_compact$adm_date)
del_tm_dur_range <- round(range((ob_data_compact$gest_age_days)/7),1)
age_range <- range(ob_data_compact$age)
