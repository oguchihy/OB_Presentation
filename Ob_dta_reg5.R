...
#cumhaz; cum-event
surv_fit <- survfit(Surv(adm_to_del_tm, event) ~ hghRsk, data = ob_data_hghRsk)
# Plotting the Survival Probability
plot(surv_fit, fun = "event", col = 1:3, main = "Survival Probability",
     xlab = "Time", ylab = "Delivery-as-Event Probability", mark.time = TRUE)
...
# Format p-values for merging with tidy data
p_values_df <- as.data.frame(p_values, check.names = FALSE)
names(p_values_df) <- rownames(p_values)
p_values_tidy <- reshape2::melt(p_values_df)
names(p_values_tidy) <- c("term", "y.level", "p.value")
library(broom)
...
tidy_model_hghRsk <- merge(tidy_model_hghRsk, p_values_tidy, by = c("term", "y.level"))
...
ggplot(tidy_model_hghRsk, aes(x = term, y = estimate, fill = y.level)) +
    ...
    geom_text(aes(label = sprintf("p = %.3f", p.value)), position = position_dodge(width = 0.9), hjust = -0.1, size = 3.5)
...
ob_data_logit_hghRsk <- ob_data_hghRsk %>%
    mutate(
        hghRsk = as.numeric(factor(hghRsk, levels = c("no", "yes"))) - 1
    )
...
# Convert high_risk to a factor for logistic regression
#ob_data_logit_hghRsk$hghRsk <- factor(ob_data_logit_hghRsk$hghRsk, levels = c(0,1))
# Fit logistic regression model
