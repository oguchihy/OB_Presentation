...
combined_data <- bind_rows(
  ...
  forecast %>% mutate(type = "Forecast", y = yhat) %>% select(ds, y, type)
)
...
Maternal age range was between ***`r age_range[1]`*** and ***`r age_range[2]`***, with a ***median age of `r median(ob_stats_block$age)`***. Birth weights and distribution are summarized below; with minimum, maximum and mean of **```r min_birth_wt```, ```r max_birth_wt```, ```r ave_birth_wt```** respectively (See plots below.)
...
if (!all(c("age", "grav", "para", "weight", "apg1", "apg5", "diff_grav_para", "gest_age_days", "adm_to_del_tm") %in% colnames(ob_data_tbl1))) {
  stop("One or more required columns are missing in ob_data_tbl1")
}

# CreateTableOne for the entire dataset
practiceTbl <- CreateTableOne(data = ob_data_tbl1)
vars <- setdiff(names(ob_data_tbl1), "gest_age")
...
print(cat_tbl)
print(cont_tbl)
print(kableone(print(strata_tbl_clst, nonnormal = "cluster", cramVars = "cluster")))
print(kableone(print(strata_tbl_hghRsk, nonnormal = "cluster", cramVars = "hghRsk")))
print(kableone(print(strata_tbl_clst_rsk, nonnormal = "hghRsk", cramVars = "cluster")))
...
##### Interactive: first select variable, then Update Table
...
ob_data_tsibble <- ob_data_ts %>%
  as_tsibble(index = week_start)

# Decompose the time series
decomposition <- ob_data_tsibble %>%
  model(STL(deliveries ~ trend(window = 13) + season(window = "periodic"))) %>%
  components()

# Plot trend and seasonality lines along with the original data
p2 <- ggplot() +
  geom_line(data = ob_data_tsibble, aes(x = week_start, y = deliveries, color = "Original Data"), size = 1) +
  geom_line(data = decomposition, aes(x = week_start, y = trend, color = "Trend"), size = 1) +
  geom_line(data = decomposition, aes(x = week_start, y = season_adjust, color = "Seasonality"), size = 1) +
...
adf_test <- ur.df(ob_data_ts$deliveries, type = "trend", selectlags = "AIC")
summary(adf_test)

# Perform Ljung-Box test on the residuals of the STL decomposition
stl_decomposition <- stl(ts(ob_data_ts$deliveries, frequency = 52), s.window = "periodic")
residuals <- stl_decomposition$time.series[, "remainder"]

ljung_box_test <- Box.test(residuals, type = "Ljung-Box")
print(ljung_box_test)

# Fit an ARIMA model
model_fit <- ob_data_tsibble %>%
