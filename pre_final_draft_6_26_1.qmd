...
- relatively only a few pregnancies under age 19; much more are at 36 and over
...
```{r shiny-server, eval=FALSE, include=FALSE, eval=FALSE}
...
The top graph shows the weekly deliveries time series seen earlier. The second and third panels tease out the series to extract seasonality information and trend, respectively. The last panel superimposes the seasonality and trend graphs on the original time series. We can see a trend that "trough-ed" around end of the first quarter of 2023 and is rising since then. The seasonality graph nearly matches the original data, indicating that there is a significant seasonal variation in weekly deliveries among our patients.

What might account for the trough? Below we compare the original (full data) trend with the trend of each of the components of various variables to look for disparities.
...
If the trend of the original data ("Overall Trend) parallels the variable's trend, the latter must be contributing significantly to the shape and direction of the former. Based on that logic, we conclude that uds_age 25_44 (age_group 21-29), non-Preterm, not-High Risk, contributed to the drop in deliveries at the trend nadir, whereas zipcode had no effect.   
...
-   We conclude that the incidence of delivery by Cesarean section and Repeat Cesarean Section is significantly higher among High Risk OB patients when compared to vaginal delivery in the same group.
...
output$xiCorPlot <- renderPlot({
  ...
  if (input$var1 != input$var2) {
    ...
    ggplot2::ggplot() + labs(title = "Please select two different variables")
  }
})
...
The analysis shows that Preterm is significantly associated with different frequencies of different Labor types, different intrapartal conditions and different intrapartal events; however, low numbers make conclusions challenging.
...
```
There's a top plot (Survival Plot) which shows the probability of Survival at any time; and the bottom plot (Cumulative Hazard Plot) which shows accumulated events at any time. 
The intersecting vertical lines in the top / Survival Plot determine "median survival (time)": the time at which survival probability is 50% (time at which half the population will have experienced the event). Time here is measured in hours since hospital admission registration.
...
We can interpret the top graph as: median survival is 12 hours; the time when half of our patients is still undelivered is 12 hours. Our patients have a 50% probability of being undelivered at 12 hours after admission. 

For the bottom graph, we determine the time when the cumulative deliveries show 50% still need to deliver or are still to deliver. It works out to 12 hours (a coincidence when compared to the median survival time of 12 hours), the accumulated "event" / "hazard" (delivery) is 41% of total. That said, the best use of the method is in comparison of different variable subsets, for example:
...
<!-- sig_event_coeff <- ggplot2::ggplot(significant_coefficients_melted_events, aes(x = reorder(Predictor, Coefficient), y = Coefficient, fill = Class)) + -->
...
<!-- # Plot the significant coefficients -->
<!-- sig_cond_coeff <- ggplot2::ggplot(significant_coefficients_melted_conditions, aes(x = reorder(Predictor, Coefficient), y = Coefficient, fill = Class)) + -->
<!--   geom_bar(stat = "identity", position = "dodge") + -->
<!--   geom_text(aes(label = round(Coefficient, 2)), position = position_dodge(width = 0.9), hjust = -0.2) + -->
<!--   facet_wrap(~Class, scales = "free") + -->
<!--   coord_flip() + -->
