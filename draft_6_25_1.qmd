---
title: "OB DATA Analytics: being informed by our Data"
...
- relatively only a few pregnancies under age 19, but many are at 36 and over
- primeps account for **```r prcnt_p_0```%**; second pregnancies **```r prcnt_p_0```%**; so slightly over half of all our deliveries are to women with 1 or 2 deliveries, counting the current pregnancy
...
p <- ggplot(data, aes(x = Date, y = Deliveries)) +
...
```
There are many other ways to analyze this data. For example, the question was posed: does labor induction lead to more C-sections?
...
kableExtra::kable(kableone(strata_tbl_del_method_rsk), format = "html")
...
```{r shiny-server, eval=FALSE, include=FALSE}
...
# Create the bar plot
p <- ggplot(zip_df_plt, aes(x = reorder(zip, freq), y = freq)) +
...
The top 5 are 93905 (one-third of all deliveries), 93906 (one-fifth), 93927 (18%); 95012 (8%), 93930 (one-twentieth). Considering its size and straddling location, 99308 is a sparse draw.
...
This map shows that zipcode 93908 sitting smack within "CSVS territory" has extremely low deliveries. Census Information[^2] reveals that this discrepancy is demographical (socioeconomic) and related to CSVS drawing pool. This zipcode ("Salinas - Corral De Tierra"):
...
In comparison, 93905 zipcode with the highest deliveries, on the other hand, has "primarily other race", with a medium household income of \$41,607 and Median Home Value of \$203,400.[^3]
...
The top graph shows the familiar weekly deliveries time series. The second and third panels tease out the series to extract seasonality information and trend, respectively. The last panel superimposes the seasonality and trend graphs on the original time series. We can see a trend that "trough-ed" around end of the first quarter of 2023 and is rising since then. The seasonality curve nearly matches the original data, indicating that there is a significant seasonal variation in weekly deliveries among our patients.

What might account for the trough? Below we compare the original (full data) trend with the trend of each of the components of various variables to look for discrepancies
...
The answer is: there is no significant difference in the relative distribution of High Risk OB, Delivery Method and Pre-term deliveries among different zip codes, based on the reported p-values.
...
output$xiCorPlot <- renderPlot({
  ...
  if (input$var1 != input$var2) {
    ...
    ggplot2::ggplot(xi_data, aes(x = Variable1, y = Spearman, fill = Variable2)) +
      geom_col() +
...
Let's now shift our attention to hospital events related to OB care. First, we will extend the previous analyses to hospital events. Then, we will use a different method of analysis known as Survival Analysis (SA) or by Time-to-Event (TTE) Analysis to probe our data. The hospital event of interest is Delivery; along with related matters of labor type, delivery method, intrapartal events and intrapartal conditions. Here are the variables and respective items:
...
We have consolidated some of the items within their respective original variables.
...
Focusing on the 3 most frequent intrapartal events, there is a significant difference in precipitous labor (93905, 93927, 93906); Prolonged Labor \>20 hrs (93905, 93906, 93927); and Prolonged 2nd Stg \>2.5 hr (93905, 93906, 93927); 93905 had the most difference in each category. There was no difference with respect to intrapartal conditions or Labor Type.
...
There are many other ways to analyze the data with this method. For example, the question was posed: does labor induction lead to more C-sections?
...
kableone(print(strata_tbl_del_method_rsk, cramVars = "lbr_type_cnsldt"), caption = "Distribution of Delivery Method by Labor type")

```

We show a statistically significant effect (p = <0.001): Induced labor is associated with a higher frequency of Primary C-Section when compared to other delivery types (36.4%), and also when primary C-section rate is compared to vaginal delivery rate for the labor induction group (36.4% Vs 25.1%).

Now, on to Survival Analysis (SA) or by Time-to-Event (TTE) Analysis.

### Survival Analysis (SA) (Shiny App)
...
We can interpret the top graph as: our patients have a 50% probability of being undelivered at 12 hours after admission. At the same 12-hour mark, the accumulated "event"hazard" (delivery) is 41% of total. That said, the best use of the method is in comparison of different variable subsets, for example:
...
The example above compares the condition of membrane-rupture to no membrane-rupture. p-value of 0.34 is a dead give-away: since it is \> 0.05, there is no statistical significant difference among the dyad with respect to probability of survival (undelivered); or cumulative hazard (delivered) at any time. Based on the graph, at 12 hours, the probability of being undelivered is 50% for either of them, while the cumulative hazard is 55% and 61% respectively Which do not meet the test of statistical significance.

On the other hand, below we can see the difference; p-value \<0.0001. 50% undelivered probability is at 12-hr mark if no Preeclampsia, but 25 hours when Preeclampsia is present. At the same time , cumulative delivered is 57% with Preeclampsia absent but only 24% if Preecalmpsia is present.
...
```
 and this example of zipcode effects showing no significant relative and comparative contribution of zipcodes to delivery status in terms of the event:
...
# Initialize an empty data frame to store all results
summary_table <- tibble(Variable = character(), Interpretation = character())
cumhaz_table <- tibble(Variable = character(), CumHazEffect = character())
...
# Apply the function to each significant variable and combine results into one data frame
results_list <- lapply(significant_vars, extract_effect)
summary_table <- bind_rows(results_list) %>% arrange(Interpretation)
...
We've explored cumulative hazard (where hazard = Delivery) which is accumulated delivery count over time at any time. Now, we will look at Risk Ratio, otherwise known as Hazard Ratio (HR) (risk in this case = Delivery): a comparative Risk between binarized variable.
...
extract_and_plot <- function(model, title, reference_levels) {
  ...
  plot <- ggplot(results, aes(x = reorder(predictor, OR), y = OR)) +
    ...
    scale_y_log10() +  # Use default limits
    labs(title = title,
         x = "Predictor",
...
This shows that the significant predictors for Events presence are: Spontaneous Augmented labor (OR 2.96); and less than 1 OR for Vaginal delivery and Repeat C/S demonstrate. 
For Conditions presence: High Risk (3.82), uds_age25-44 (1.96); with less than 1 OR for age (0.94) Vaginal delivery (0.68), zip codes 93905 (0.50), 93930 (0.40), Spontaneous Labor (0.34) 

A summary table for all the outcomes we are interested in and the the variables follows.
...
<!-- sig_event_coeff <- ggplot(significant_coefficients_melted_events, aes(x = reorder(Predictor, Coefficient), y = Coefficient, fill = Class)) + -->
...
<!-- # Save the plot with adjusted dimensions if needed -->
<!-- ggsave("sig_event_coeff_plot.png", plot = sig_event_coeff, width = 11, height = 6) -->
<!-- sig_event_coeff -->
...
<!-- kableExtra::kable(significant_vars_events, caption = "Significant Coefficients for Intrapartal Events") -->
...
<!-- # Plot the significant coefficients -->
<!-- sig_cond_coeff <- ggplot(significant_coefficients_melted_conditions, aes(x = reorder(Predictor, Coefficient), y = Coefficient, fill = Class)) + -->
<!--   geom_bar(stat = "identity", position = "dodge") + -->
<!--   geom_text(aes(label = round(Coefficient, 2)), position = position_dodge(width = 0.9), hjust = -0.2) + -->
<!--   facet_wrap(~Class, scales = "free") + -->
<!--   coord_flip() + -->
...
<!-- # Save the plot with adjusted dimensions if needed -->
<!-- ggsave("sig_cond_coeff_plot.png", plot = sig_cond_coeff, width = 12, height = 6) -->
<!-- sig_cond_coeff -->
<!-- # Extract significant predictors -->
...
<!-- kableExtra::kable(significant_vars_conditions, caption = "Significant Coefficients for Intrapartal Conditions") -->
...
extract_and_plot <- function(model, title) {
  ...
  if (nrow(significant_results) > 0) {
    # Plot the significant coefficients
    plot <- ggplot(significant_results, aes(x = reorder(predictor, coefficient), y = coefficient, fill = predictor)) +
      geom_bar(stat = "identity", position = "dodge") +
...
We will selct the top 20 variables for each outcome in reference to Intrapartal Events and Intrapartal Conditions.
...
var_imp_event <- ggplot(importance_df_events, aes(x = reorder(Variable, MeanDecreaseGini), y = MeanDecreaseGini, fill = MeanDecreaseGini)) +
...
var_imp_cond <- ggplot(importance_df_conditions, aes(x = reorder(Variable, MeanDecreaseGini), y = MeanDecreaseGini, fill = MeanDecreaseGini)) +
  geom_bar(stat = "identity") +
  scale_fill_gradient(low = "blue", high = "red") +
  coord_flip() +
...
# Interactive Analyses Summary

```{r echo=FALSE, warning=FALSE, message=FALSE}
source(file = "partiallyWrkingRShiny5_27_update_6_21.R")
