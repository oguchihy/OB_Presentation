...
We can interpret the top graph as: median survival is 12 hours; the time when half of our patients is expected to be still undelivered is 12 hours. Our patients have a 50% probability of being undelivered at 12 hours after admission. 

For the bottom graph, we determine the time when the cumulative deliveries show 50% still need to deliver or are still to deliver. It works out to about 12 hours (a coincidence when compared to the median survival time of 12 hours)

This method is most illuminating when used to compare of different variable subsets, for example:
...
The example above compares the condition of membrane-rupture to no membrane-rupture. p-value of 0.34 is a dead give-away: since it is \> 0.05, there is no statistical significant difference among the dyad with respect to probability of survival (undelivered); or cumulative hazard (total delivered) at any time. Based on the graph, at 12 hours, the probability of being undelivered is 50% for either of them, while the cumulative hazard is 55% and 61% respectively, which do not meet the test of statistical significance for being different.

On the other hand, below we can see the difference; p-value \<0.0001. 50% undelivered probability is at ~12-hr mark if no Preeclampsia, but 25 hours when Preeclampsia is present. At the same time , cumulative delivered is 57% with Preeclampsia absent but only 24% if Preeclampsia is present.
