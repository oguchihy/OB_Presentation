...
vars <- c("lbr_type_cnsldt", "intrapartal_events", "presentation_cnsldt", "conditions_cnsldt", "del_method_cnsldt")

selected_hosp <- tableone::CreateTableOne(data = ob_data_tbl1, vars = vars)

#strata_tbl_selected <- CreateTableOne(data = ob_data_tbl1, vars = c( "zip")
kableone(print(selected_hosp), caption = "Distribution of Hosipital Variables")
...
suppressWarnings({
model <- glm(binary_outcome ~ zip_category + age + grav + para + gender + uds_age + hghRsk + del_method_cnsldt + lbr_type_cnsldt, data = ob_data_fctr, family = binomial(), )
...
ggplot(tidy_model, aes(x = reorder(term, estimate), y = estimate)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Logistic Regression Coefficients with P-values -- Preterm outcome",
       x = "Predictors",
       y = "Coefficient Estimate") +
...
# Fit the logistic regression model (assuming model is already fitted as in your context)
suppressWarnings({
model_hgh <- glm(binary_outcome_hgh ~ zip_category + age + grav + para + gender + uds_age + del_method_cnsldt + cluster + lbr_type_cnsldt, data = ob_data_fctr, family = binomial())
})
#summary(model_hgh)
...
ggplot(tidy_model, aes(x = reorder(term, estimate), y = estimate)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Logistic Regression Coefficients with P-values -- High Risk OB",
       x = "Predictors",
       y = "Coefficient Estimate") +
  theme(axis.text.y = element_text(size = 10)) +
  ylim(c(-1, 1))  # Adjust this range as needed based on your data
