...
# Eliminate cases where grav == 1 & para == 0
ob_data_filtered <- ob_data_tbl_xtra_initial %>%
  filter(!(grav == 1 & para == 0))
...
ob_data_preg_without_delivery <- ob_data_corrected %>%
  mutate(preg_without_delivery = grav - corrected_para)

# Summarize the data by age group
summary_data <- ob_data_preg_without_delivery %>%
  group_by(age_group) %>%
  summarise(total_pregnancies = sum(grav),
            total_deliveries = sum(corrected_para),
...
mean_gap <- round(mean(summary_data$incidence_rate, 0))

```

For our patient population, excluding those who are G1P0 for the current pregnancy, the average ```r mean_gap```% of pregnancies were not delivered; 47% for 29 and under age-groups and 35% for over 29 age groups. This figure is calculated by adding the gravidarity numbers and substracting parity numbers, while accounting for multiple gestations.
...
ggplot(summary_data, aes(x = age_group, y = incidence_rate, fill = age_group)) +
  ...
  annotate("text", x = 1, y = max(summary_data$incidence_rate) + 5, 
           label = "n/n1: total_preg_without_delivery / total_pregnancies", 
           hjust = 0, vjust = 1, size = 4, color = "black") +
  theme_minimal() +
  theme(legend.position = "none")

mean_incidence_rate <- mean(summary_data$incidence_rate)
