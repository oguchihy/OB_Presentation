...
# Print the table using kableone and style it
# kableone(print(selected_hosp), caption = "Distribution of Hospital Variables", format = "html") %>%
#   kable_styling(full_width = FALSE) 



# Convert to data frame
# selected_hosp_df <- as.data.frame(print(selected_hosp))
# 
# # Print the table to HTML
# table_html <- kableExtra::kable(selected_hosp_df, format = "html", caption = "Distribution of Hospital Variables", table.attr = "class='table' style='font-size: 12px; width: auto !important;'") %>%
...
selected_hosp_df <- selected_hosp_df %>%
  mutate(
    Header = ifelse(grepl("^X\\.*", RowID), "", RowID),
    Item = ifelse(grepl("^X\\.*", RowID), gsub("^X\\.*", "", RowID), ""),
    Value = Overall
  ) %>%
  select(Header, Item, Value) %>%
