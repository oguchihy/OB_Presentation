...
library(survminer)
library(ggplot2)
library(myRFunctions)  # Includes ggforest3
library(nnet)  # For multinom()
library(dplyr)
library(randomForest)
...
# Model without strata for HR
fitModelHR <- eventReactive(input$updateModel, {
  formulaHR <- as.formula(paste("Surv(time, event) ~", input$predictorHR))
  modelHR <- coxph(formulaHR, data = ob_data)
  print(summary(modelHR))  # Debugging model output
  return(modelHR)
})
...
fitVarImp <- eventReactive(input$updateModel, {
...
output$survPlot <- renderPlot({
  fit <- fitModel()
  surv_fit <- survfit(fit)  # Use 'fit' here, not 'model'
