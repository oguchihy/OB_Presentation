...
- primips account for **```r prcnt_p_0```%**; second pregnancies **```r prcnt_p_0```%**; so slightly over half of all our deliveries are to women with 1 or 2 deliveries, counting the current pregnancy
...
For our patient population, annually, an average  of **```r overall_gap_annual```%** of pregnancies were not delivered; 21-29 age_group was highest at 17% while the rest were even at 15%. Numbers were derived by adding the gravidarity numbers and subtracting parity numbers, while crediting one delivery to G1P0; and accounting for multiple gestation.
...
```
Focusing on first deliveries only, for every 4 delivered pregnancies, one pregnancy was not delivered, given an overall not-delivered incidence rate of  **```r overall_incidence_rate```%**.
