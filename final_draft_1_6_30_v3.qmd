...
Cesarean section rate was **`r c_section_prcnt`%** for Primary, constituting **`r prim_c_section_prcnt_all.cs`%** of all the sections.  There was **1 Fetal Demise** during the period.
...
kableone(print(strata_tbl_grav_zip, cramVars = "grav_fct"), caption = "Distribution of Gravidarity by Zip code")

strata_tbl_para_zip <- CreateTableOne(data = ob_data_tbl_xtra, strata = "para_fct", vars = "zip")
kableone(print(strata_tbl_para_zip, cramVars = "para_fct"), caption = "Distribution of Parity by Zip code")

strata_tbl_age_cat_para_zip <- CreateTableOne(data = ob_data_tbl_xtra, strata = "age_group", vars = "zip")
kableone(print(strata_tbl_age_cat_para_zip, cramVars = "age_group"), caption = "Distribution of Age-groups by Zip Code")

strata_tbl_age_cat_grav_zip <- CreateTableOne(data = ob_data_tbl_xtra, strata = "age_group", vars = "zip")
kableone(print(strata_tbl_age_cat_grav_zip, cramVars = "grav_fct"), caption = "Distribution of Age-groups by Zip Code")


strata_tbl_age_cat_para <- CreateTableOne(data = ob_data_tbl_xtra, strata = "age_group", vars = "para_fct")
kableone(print(strata_tbl_age_cat_para, cramVars = "age_group"), caption = "Distribution of Age-groups by Parity")

strata_tbl_age_cat_grav <- CreateTableOne(data = ob_data_tbl_xtra, strata = "age_group", vars = "grav_fct")
kableone(print(strata_tbl_age_cat_grav, cramVars = "grav_fct"), caption = "Distribution of Age-groups by Gravidarity")
```
...
ob_data_filtered <- ob_data_tbl_xtra_initial %>%
  mutate(corrected_para = if_else(baby_sq > 1, para - 1, para),
         para.xtr= if_else(corrected_para==0 & grav==1, 1, corrected_para),
...
ob_data_preg_without_delivery <- ob_data_filtered%>%
  mutate(preg_without_delivery = grav.xtr - para.xtr)

# Summarize the data by age group
summary_data <- ob_data_preg_without_delivery %>%
  group_by(age_group) %>%
  summarise(total_pregnancies = sum(grav.xtr),
...
kable(summary_data, caption = "Incidence of Pregnancies without Delivery by Age Group")

# Plot the incidence rates by age group
ggplot(summary_data, aes(x = age_group, y = annual_incidence_rate, fill = age_group)) +
...
overall_gap_annual  <- round((sum(summary_data$total_preg_without_delivery)/sum(summary_data$total_pregnancies))*100/9*4,0)

```

For our patient population, annually, an average  of **```overall_gap_annual```%*** of pregnancies were not delivered; 21-29 age_group was highest at 17% while the rest were even at 15%. Numbers were derived by adding the gravidarity numbers and substracting parity numbers, while crediting one delivery to G1P0; and accounting for multiple gestations.

For the current pregnancy alone, here is the outcome:
...
# Display the summary table
kable(summary_data, caption = "Incidence of Pregnancies without Delivery **for Primeps** by Age Group")
...
```
First deliveries represent only **```r 1-overall_incidence_rate```%** of the pregnancies; **```r overall_incidence_rate```%** was not delivered
...
trend_results_list <- lapply(variables, function(var) {
  calculate_trend_tests(ob_data_fctr, var)
})
...
trend_results <- trend_results %>%
  mutate(variable_level = paste(variable, coalesce(zip, hghRsk, cluster, age_group, uds_age), sep = ": ")) %>%
...
We can interpret the top graph as: median survival is 12 hours; the time when half of our patients is __expected__ to be still undelivered is 12 hours. Our patients have a 50% probability of being undelivered at 12 hours after admission. 

For the bottom graph, we determine the time when the cumulative deliveries show 50% still need to deliver (or are still to deliver). It works out to be about 12 hours -- just a coincidence that the median survival time is also 12 hours)
...
The example above compares the condition of __membrane-rupture to no membrane-rupture__. First thing to check is the p-value. A p-value of 0.34 is \> 0.05, there is therefore no statistically significant difference among the binary with respect to probability of survival (undelivered); or cumulative hazard (total delivered) at any time. Based on the graph, at 11 hours, the probability of being undelivered is 50% for either of them. At 12 hours (your pick) the cumulative hazard (accumulated deliveries) is 55% and 61% respectively, which do not meet the test of statistical significance for being different.
...
We have similar plots of all the outcomes we are interested in, but will summarize the findings in a table, pared down to only the statistically significant. 
...
Summary of results: The multinomial logistic regression model was used to analyze the relationship between various predictors and the occurrence of either Intrapartal Events or Intrapartal conditions. The outcome variable was re-coded to model the presence of intrapartal events as "Yes" versus "None"; same for intrapartal conditions. Our interest is in the coefficients.
...
We will engage one more analysis schema, looking for Variable Importance, because we may use our data to make forecasts.
