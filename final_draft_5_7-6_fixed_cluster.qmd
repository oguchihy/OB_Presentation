...
ob_data_tbl_xtra <- readRDS("All Factored Complete Ready OB Dataset for Analytics.RDS") %>% 
  ...
  mutate(cluster = if_else(gest_age_days <=259, 1, 0)) %>% 
...
Is there a difference in the distribution of different age-groups within each zip code? 
...
The answer is, no, based on the p-values
...
No significant difference in the distribution of __Gravidarity__ within each zip code, but __Parity__ shows

- significantly higher Primips within 93905; 

- significantly higher para 3 and over within 93927
...
With a p-value \<0.001, a statistically significant difference is obvious in the distribution of Cesarean sections compared to Vaginal deliveries among High Risk OB. The same holds true for Preterm: a significantly higher percent are in High Risk group than non-Preterm (39.8% Vs 19.2%, p-value \<0.001).
...
Here, the distribution of C-sections is significantly higher compared with vaginal delivery among Preterm deliveries.

Conclusion: 
- The distribution of the outcome, Delivery Method, is significantly different among High Risk OB, with delivery by Cesarean Sections (44.5% Primary; 32.8 Repeat) than vaginal delivery (15.6%). 

- Preterm has a higher distribution among High Risk OB than non-High Risk OB.

- Significantly higher distribution of C-sections compared with vaginal delivery in Preterm group.
