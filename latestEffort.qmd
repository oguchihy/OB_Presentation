...
fitModel <- eventReactive(input$updateModel, {
    ...
    tryCatch({
        model <- coxph(formula, data = ob_data_tte)
        print(summary(model))  # Debugging model output
        return(model)
    }, error = function(e) {
        showNotification(paste("Error in Cox model:", e$message), type = "error")
        return(NULL)
    })
})
...
fitMultinom <- eventReactive(input$updateModel, {
    ...
    tryCatch({
        ...
        showNotification(paste("Error in Multinomial model:", e$message), type = "error")
        return(NULL)
    })
})
...
fitOR <- eventReactive(input$updateModel, {
    ...
    tryCatch({
        ...
        showNotification(paste("Error in Odds Ratio model:", e$message), type = "error")
        return(NULL)
    })
})
...
output$varImportancePlot <- renderPlot({
    # Exclude high-cardinality variables and other unsuitable predictors
    data_for_rf <- ob_data_tte %>%
        select(-one_of("zip"))  # Replace with actual high-cardinality variable names
    ...
    tryCatch({
        ...
        showNotification(paste("Error in Variable Importance model:", e$message), type = "error")
        return(NULL)
    })
})
