...
ob_data_cnsldt <- readRDS("OB_data with High Risk OB.RDS") %>% 
  ...
  mutate(
    ...
    del_method_cnsldt = case_when(
      str_detect(delivery_method, "Spont") |str_detect(delivery_method, "Vacuum") | str_detect(delivery_method, "Fetal") | str_detect(delivery_method, "vBAC") ~ "Vaginal",
      str_detect(delivery_method, "Prim") ~ "C/S, Primary",
      str_detect(delivery_method, "Repeat") | str_detect(delivery_method, "Failed vBAC") ~ "C/S, Repeat",
